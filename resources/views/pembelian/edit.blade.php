...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Edit Permintaan Pembelian #{{ $pembelian->custom_number ?? $pembelian->id }}
            </h1>
        ...
        <form action="{{ route('pembelian.update', $pembelian->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card shadow mb-4">
                <div class="card-body">
                    ...
                    <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control @error('tgl_transaksi') is-invalid @enderror"
                                            ...
                                            value="{{ old('tgl_transaksi', $pembelian->tgl_transaksi->format('Y-m-d')) }}"
                                            required>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control bg-light" id="tgl_jatuh_tempo_display"
                                            readonly>
                                        <input type="hidden" id="tgl_jatuh_tempo" name="tgl_jatuh_tempo"
                                            value="{{ old('tgl_jatuh_tempo', $pembelian->tgl_jatuh_tempo ? $pembelian->tgl_jatuh_tempo->format('Y-m-d') : '') }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <div class="form-group">
                                ...
                                @else
                                    <input type="text" class="form-control" value="{{ $pembelian->gudang->nama_gudang ?? '-' }}"
                                        readonly>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('tahun_anggaran') is-invalid @enderror"
                                    ...
                                    value="{{ old('tahun_anggaran', $pembelian->tahun_anggaran) }}">
                                @error('tahun_anggaran') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="form-group">
                                <label for="tag">Tag</label>
                                <input type="text" class="form-control" id="tag" name="tag"
                                    value="{{ old('tag', $pembelian->tag) }}" readonly>
                            </div>
                            <div class="form-group">
                                ...
                                @foreach($items as $index => $item)
                                    ...
                                    <tr>
                                            ...
                                            <select class="form-control product-select" name="produk_id[]" required>
                                                ...
                                                @foreach($produks as $produk)
                                                    <option value="{{ $produk->id }}" data-harga="{{ $produk->harga }}"
                                                        ...
                                                        data-satuan="{{ $produk->satuan ?? 'Pcs' }}" {{ $oldProdukId == $produk->id ? 'selected' : '' }}>
                                                        {{ $produk->nama_produk }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <td><input type="text" class="form-control product-description" name="deskripsi[]"
                                                value="{{ $oldDeskripsi }}"></td>
                                        <td><input type="number" class="form-control product-quantity" name="kuantitas[]"
                                                value="{{ $oldKuantitas }}" min="1" required></td>
                                        <td>
                                            <input type="text" class="form-control product-unit" name="unit[]"
                                                value="{{ $oldUnit }}" readonly>
                                        </td>
                                        <td><input type="number" class="form-control text-right product-price"
                                                name="harga_satuan[]" value="{{ $oldHarga }}" placeholder="0" required></td>
                                        <td><input type="number" class="form-control text-right product-discount"
                                                name="diskon[]" value="{{ $oldDiskon }}" placeholder="0" min="0" max="100"></td>
                                        <td><input type="text" class="form-control text-right product-line-total"
                                                name="jumlah[]" placeholder="0" readonly></td>
                                        <td>
                    ...
                    @error('produk_id.*') <div class="text-danger small mt-2">Error di baris Produk: {{ $message }}</div>
                    @enderror
                    @error('kuantitas.*') <div class="text-danger small mt-2">Error di baris Kuantitas: {{ $message }}</div>
                    @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('memo') is-invalid @enderror" id="memo" name="memo"
                                    rows="4">{{ old('memo', $pembelian->memo) }}</textarea>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" class="form-control text-right" id="diskon_akhir_input"
                                                name="diskon_akhir"
                                                value="{{ old('diskon_akhir', $pembelian->diskon_akhir) }}">
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td style="width: 50%;">
                                            <input type="number"
                                                ...
                                                value="{{ old('tax_percentage', $pembelian->tax_percentage) }}" min="0"
                                                step="0.01">
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const productOptionsHtml = `@foreach($produks as $produk)<option value="{{ $produk->id }}" data-harga="{{ $produk->harga }}" data-deskripsi="{{ $produk->deskripsi }}" data-satuan="{{ $produk->satuan ?? 'Pcs' }}">{{ $produk->nama_produk }}</option>@endforeach`;
            ...
            <select class="form-control product-select" name="produk_id[]" required>
                <option value="">Pilih...</option>
                @foreach($produks as $produk)
                    <option value="{{ $produk->id }}" data-harga="{{ $produk->harga }}" data-deskripsi="{{ $produk->deskripsi }}" data-satuan="{{ $produk->satuan ?? 'Pcs' }}">
                        {{ $produk->nama_produk }}
                    </option>
                @endforeach
            </select>
