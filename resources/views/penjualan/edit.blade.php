...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Penagihan Penjualan #{{ $penjualan->custom_number ?? $penjualan->id }}</h1>
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                ...
                <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="kontak-select" name="pelanggan" required>
                                        ...
                                        @foreach($kontaks as $kontak)
                                            <option value="{{ $kontak->nama }}" data-email="{{ $kontak->email }}" data-alamat="{{ $kontak->alamat }}" data-diskon="{{ $kontak->diskon_persen }}" {{ old('pelanggan', $penjualan->pelanggan) == $kontak->nama ? 'selected' : '' }}>{{ $kontak->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group"><label>Email</label><input type="email" class="form-control" id="email-input" name="email" value="{{ old('email', $penjualan->email) }}"></div>
                            </div>
                        </div>
                        <div class="form-group"><label>Alamat Penagihan</label><textarea class="form-control" id="alamat-input" name="alamat_penagihan" rows="2">{{ old('alamat_penagihan', $penjualan->alamat_penagihan) }}</textarea></div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group"><label>No Transaksi</label><input type="text" class="form-control" value="{{ $penjualan->custom_number ?? '[Auto]' }}" disabled></div>
                        <div class="form-group"><label>No. Referensi</label><input type="text" class="form-control" name="no_referensi" value="{{ old('no_referensi', $penjualan->no_referensi) }}"></div>
                        <div class="form-group"><label>Tag</label><input type="text" class="form-control" name="tag" value="{{ old('tag', $penjualan->tag) }}" readonly></div>
                        
                        <div class="form-group">
                            <label>Gudang *</label>
                            @if(in_array(auth()->user()->role, ['admin', 'super_admin']))
                                <select class="form-control" name="gudang_id" required>
                                    @foreach($gudangs as $g) <option value="{{ $g->id }}" {{ old('gudang_id', $penjualan->gudang_id) == $g->id ? 'selected' : '' }}>{{ $g->nama_gudang }}</option> @endforeach
                                </select>
                            @else
                                <input type="text" class="form-control" value="{{ $penjualan->gudang->nama_gudang ?? '-' }}" readonly>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="approver_id" required>
                                @foreach($approvers as $a) <option value="{{ $a->id }}" {{ old('approver_id', $penjualan->approver_id) == $a->id ? 'selected' : '' }}>{{ $a->name }}</option> @endforeach
                            </select>
                        </div>
                    </div>
                </div>
                                ...
                                <tr>
                                    <td>
                                        <select class="form-control product-select" name="produk_id[]" required>
                                            ...
                                            @foreach($produks as $p) <option value="{{ $p->id }}" data-harga="{{ $p->harga }}" data-deskripsi="{{ $p->deskripsi }}" {{ $oldProd == $p->id ? 'selected' : '' }}>{{ $p->nama_produk }}</option> @endforeach
                                        </select>
                                    </td>
                                    <td><input type="text" class="form-control product-description" name="deskripsi[]" value="{{ $oldDesc }}"></td>
                                    <td><input type="number" class="form-control product-quantity" name="kuantitas[]" value="{{ $oldQty }}" min="1" required></td>
                                    <td><select class="form-control" name="unit[]"><option {{ $oldUnit=='Pcs'?'selected':'' }}>Pcs</option><option {{ $oldUnit=='Box'?'selected':'' }}>Box</option></select></td>
                                    <td><input type="number" class="form-control text-right product-price" name="harga_satuan[]" value="{{ $oldPrice }}" required></td>
                                    <td><input type="number" class="form-control text-right product-discount" name="diskon[]" value="{{ $oldDisc }}" min="0" max="100"></td>
                                    <td><input type="text" class="form-control text-right product-line-total" readonly></td>
                                    <td>@if($index > 0 || count($items) > 1)<button type="button" class="btn btn-danger btn-sm remove-row-btn">X</button>@endif</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="row mt-3">
                    <div class="col-md-6">
                        <div class="form-group"><label>Memo</label><textarea class="form-control" name="memo" rows="3">{{ old('memo', $penjualan->memo) }}</textarea></div>
                        <div class="form-group"><label>Lampiran</label><input type="file" class="form-control-file" name="lampiran"></div>
                    </div>
                    <div class="col-md-6">
                        <table class="table table-borderless text-right">
                            <tr><td><strong>Subtotal</strong></td><td id="subtotal-display">Rp0</td></tr>
