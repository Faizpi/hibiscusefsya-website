        ...
        <div class="d-flex align-items-center justify-content-between mb-4 page-header-mobile">
            <div class="detail-title-block">
                ...
                <div class="detail-meta">
                    <span class="detail-meta-item"><i class="fas fa-circle"></i><strong>{{ $penjualan->status_display ?? $penjualan->status }}</strong></span>
                    <span class="detail-meta-item"><i class="fas fa-calendar-alt"></i>{{ $penjualan->tgl_transaksi->format('d M Y') }}</span>
                    <span class="detail-meta-item"><i class="fas fa-warehouse"></i>{{ $penjualan->gudang->nama_gudang ?? '-' }}</span>
                </div>
            </div>
            <div class="show-action-buttons">
                ...
                {{-- Tombol Uncancel (Hanya super_admin) --}}
                @if($penjualan->status == 'Canceled' && $user->role == 'super_admin')
                    <button type="button" class="btn btn-info btn-sm shadow-sm" data-toggle="modal"
                        data-target="#uncancelModal">
                        <i class="fas fa-undo fa-sm"></i> Batalkan Pembatalan
                    </button>
                @endif
    ...
    <div class="modal fade" id="uncancelModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button class="btn btn-secondary" type="button" data-dismiss="modal">Tidak</button>
                    <form action="{{ route('penjualan.uncancel', $penjualan->id) }}" method="POST">
                        @csrf
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title"><i class="fas fa-qrcode mr-2"></i>QR Code Invoice</h5>
                    <button class="close text-white" type="button" data-dismiss="modal"><span>×</span></button>
                </div>
                <div class="modal-body text-center">
                    <p class="mb-3">Scan QR Code di bawah untuk melihat invoice:</p>
                    @php
                        $publicUrl = route('public.invoice.penjualan', $penjualan->uuid);
