        ...
        <div class="d-flex align-items-center justify-content-between mb-4 page-header-mobile">
            <div class="detail-title-block">
                ...
                <div class="detail-meta">
                    <span class="detail-meta-item"><i class="fas fa-circle"></i><strong>{{ $biaya->status }}</strong></span>
                    <span class="detail-meta-item"><i class="fas fa-calendar-alt"></i>{{ $biaya->tgl_transaksi->format('d M Y') }}</span>
                    <span class="detail-meta-item"><i class="fas fa-warehouse"></i>{{ $biaya->gudang->nama_gudang ?? '-' }}</span>
                </div>
            </div>
                                ...
                                @foreach($allLampiran as $index => $path)
                                    ...
                                    <div class="col-6 col-md-4 mb-3 text-center lampiran-item" data-path="{{ $path }}">
                                        ...
                                        @if(Auth::user()->role === 'super_admin')
                                            <form
                                                action="{{ route('biaya.deleteLampiran', ['biaya' => $biaya->id, 'index' => $loop->index]) }}"
