...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-box h3 { background: #667eea; color: #fff; padding: 8px 12px; font-size: 12px; margin-bottom: 10px; }
        .info-row { padding: 5px 12px; }
        .info-row .label { display: inline-block; width: 100px; color: #666; }
        .info-row .value { font-weight: 500; }
        table.items { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        table.items th { background: #667eea; color: #fff; padding: 10px 8px; text-align: left; font-size: 11px; }
        ...
        .totals .grand { background: #667eea; color: #fff; padding: 12px; font-size: 14px; font-weight: bold; }
...
<body>
    ...
    <div class="info-section">
        <div class="info-box">
            ...
            <div class="info-row"><span class="label">Tanggal</span><span class="value">{{ $penjualan->tgl_transaksi->format('d F Y') }}</span></div>
            <div class="info-row"><span class="label">Waktu</span><span class="value">{{ $penjualan->created_at->format('H:i') }} WIB</span></div>
            <div class="info-row"><span class="label">Jatuh Tempo</span><span class="value">{{ $penjualan->tgl_jatuh_tempo ? $penjualan->tgl_jatuh_tempo->format('d F Y') : '-' }}</span></div>
            <div class="info-row"><span class="label">Pembayaran</span><span class="value">{{ $penjualan->syarat_pembayaran ?? '-' }}</span></div>
        ...
        <div class="info-box">
            ...
            <div class="info-row"><span class="label">Nama</span><span class="value">{{ $penjualan->pelanggan }}</span></div>
            <div class="info-row"><span class="label">Email</span><span class="value">{{ $penjualan->email ?? '-' }}</span></div>
            <div class="info-row"><span class="label">Sales</span><span class="value">{{ $penjualan->user->name }}</span></div>
            <div class="info-row"><span class="label">Gudang</span><span class="value">{{ $penjualan->gudang->nama_gudang ?? '-' }}</span></div>
        </div>
    </div>
    ...
    <div class="totals">
        ...
        <div class="row"><span class="label">Diskon</span><span class="value">- Rp {{ number_format($penjualan->diskon_akhir, 0, ',', '.') }}</span></div>
        @endif
        @if($penjualan->tax_percentage > 0)
        <div class="row"><span class="label">Pajak ({{ $penjualan->tax_percentage }}%)</span><span class="value">Rp {{ number_format($pajakNominal, 0, ',', '.') }}</span></div>
        @endif
        <div class="row grand"><span class="label">GRAND TOTAL</span><span class="value">Rp {{ number_format($penjualan->grand_total, 0, ',', '.') }}</span></div>
    </div>
